...
// 1. Check Submissions
const sub = db.prepare("SELECT * FROM submissions WHERE id = ?").get(validId);
...
let pub = db.prepare("SELECT * FROM public_chains WHERE genesis_certificate_id = ?").get(validId);
...
pub = db.prepare("SELECT * FROM public_chains WHERE genesis_certificate_id = ?").get(validId);
